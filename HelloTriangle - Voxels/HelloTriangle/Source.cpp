...
//#endif
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glClearColor(0.15f, 0.15f, 0.15f, 1.0f); //cor de fundo
		...
		else
		{
			...
			view = glm::lookAt(topViewPos, // Posição (ponto) 
				glm::vec3(0.0f, 0.0f, 2.0f), // Target (ponto, não vetor) -> dir = target - pos                
				topViewUp);// Up (vetor)
		}
			...
			for (int y = 0; y < 5; y++)
				for (int z = 0; z < 5; z++)
				{
					...
					if (iVoxel >= 0)
					{
						...
						shader.setVec4("inputColor", colorPalette[iVoxel].r, colorPalette[iVoxel].g, colorPalette[iVoxel].b, 0.5);
		...
		//Desenho de Indicador de cor
		shader.setVec4("inputColor", colorPalette[iColor].r, colorPalette[iColor].g, colorPalette[iColor].b, 1);
		model = glm::mat4(1);
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (key == GLFW_KEY_DOWN || key == GLFW_KEY_KP_2 && action == GLFW_PRESS)
	{
			gridCursor.z = ((int)gridCursor.z + 1) % 5;
	}

	if (key == GLFW_KEY_RIGHT || key == GLFW_KEY_KP_6 && action == GLFW_PRESS)
	{
		gridCursor.x = ((int)gridCursor.x + 1) % 5;
	}
	if (key == GLFW_KEY_LEFT || key == GLFW_KEY_KP_4 && action == GLFW_PRESS)
	{
		gridCursor.x = ((int)gridCursor.x - 1 + 5) % 5;
	}

	if (key == GLFW_KEY_I || key == GLFW_KEY_KP_7 && action == GLFW_PRESS)
	{
		gridCursor.y = ((int)gridCursor.y + 1) % 5;
	}
	if (key == GLFW_KEY_K || key == GLFW_KEY_KP_1 && action == GLFW_PRESS)
	{
	...
	if (key == GLFW_KEY_V && action == GLFW_PRESS)
	{
		...
		glm::lookAt(cameraPos, cameraFront, cameraUp);
...
bool load(string fileName)
{
	...
	if (!inputFile)
	{
		cout << "Error while opening save file";
		return false;
	}
	string line;
	//int n;
